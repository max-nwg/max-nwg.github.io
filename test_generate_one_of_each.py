...
def main() -> int:
	...
	# Track which types we've already generated
	generated_types = set()
	rows_processed = 0
	skipped_rows = 0
	
	# Create progress bar
	with tqdm(desc="Generating test pages (one of each type)", unit="page") as pbar:
		for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=False), start=2):
			...
			if type_value in generated_types:
				continue
			...
			pbar.set_description(f"Processing {type_value} - {rel_path}")
			...
			generated_types.add(type_value)
			rows_processed += 1
			
			# Update progress bar
			pbar.update(1)
			
			# Stop if we've generated all types (a-f)
			if len(generated_types) >= 6:
				break

	print(f"\nGenerated {rows_processed} test pages (one of each type) into {OUTPUT_DIR}")
	print(f"Generated types: {sorted(generated_types)}")
	if skipped_rows > 0:
